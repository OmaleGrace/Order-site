...
func TestUpdateCartQuantityIncrease(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	...
	err := db.QueryRow(`
		INSERT INTO users (name, email, password)
		VALUES ('Increase User', 'increase@example.com', 'hashed-password')
		RETURNING id
	`).Scan(&userID)
	if err != nil {
		t.Fatal(err)
	...
	req := httptest.NewRequest(http.MethodPost, "/cart/update", nil)
	req.Form = map[string][]string{
		"id":     {strconv.Itoa(menuItemID)},
		"action": {"increase"},
	}
...
func TestUpdateCartQuantityDecrease(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	...
	err := db.QueryRow(`
		INSERT INTO users (name, email, password)
		VALUES ('Decrease User', 'decrease@example.com', 'hashed-password')
		RETURNING id
	`).Scan(&userID)
	if err != nil {
		t.Fatal(err)
...
func TestUpdateCartQuantityDecreaseAtMinimum(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	...
	err := db.QueryRow(`
		INSERT INTO users (name, email, password)
		VALUES ('Minimum User', 'minimum@example.com', 'hashed-password')
		RETURNING id
	`).Scan(&userID)
	if err != nil {
		t.Fatal(err)
	...
	location := w.Header().Get("Location")
	if location != "/cart?message=Minimum%20order%20quantity%20is%201" {
		t.Fatalf("unexpected redirect location: %s", location)
	}
	...
	if quantity != 1 {
		t.Fatalf("expected quantity to remain 1, got %d", quantity)
	}
}

func TestUpdateCartQuantityInvalidAction(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	...
	err := db.QueryRow(`
		INSERT INTO users (name, email, password)
		VALUES ('Invalid Action User', 'invalidaction@example.com', 'hashed-password')
		RETURNING id
	`).Scan(&userID)
	if err != nil {
		t.Fatal(err)
...
func TestUpdateCartQuantityNoSession(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
...
func TestUpdateCartQuantityWrongMethod(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()

	h := New(db)

	req := httptest.NewRequest(http.MethodGet, "/cart/update", nil)
	w := httptest.NewRecorder()
